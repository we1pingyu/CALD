    ...
    with torch.no_grad():
        ...
        for images, _ in unlabeled_loader:
            ...
            for image in images:
                ...
                # if len(ref_boxes) < 3:
                #     continue
                if torch.min(torch.topk(ref_scores, 2)[0]) < 0.7 or image.size[0] < image.size[1] or torch.min(
                        torch.topk(ref_scores, 3)[0]) > 0.7 or ref_labels[torch.topk(ref_scores, 2)[1][0]] == \
                        ref_labels[torch.topk(ref_scores, 2)[1][1]] \
                        or calcu_iou(ref_boxes[torch.topk(ref_scores, 2)[1][0]],
                                     ref_boxes[torch.topk(ref_scores, 2)[1][1]]) > 0.001 or (
                        ref_boxes[torch.topk(ref_scores, 2)[1][0]][2] - ref_boxes[torch.topk(ref_scores, 2)[1][0]][
                    0]) * (ref_boxes[
                               torch.topk(ref_scores, 2)[1][0]][3] - ref_boxes[torch.topk(ref_scores, 2)[1][0]][
                               1]) < 5000 or (
                        ref_boxes[torch.topk(ref_scores, 2)[1][1]][2] - ref_boxes[torch.topk(ref_scores, 2)[1][1]][
                    0]) * (ref_boxes[
                               torch.topk(ref_scores, 2)[1][1]][3] - ref_boxes[torch.topk(ref_scores, 2)[1][1]][
                               1]) < 5000:
                    print((
                                  ref_boxes[torch.topk(ref_scores, 2)[1][0]][2] -
                                  ref_boxes[torch.topk(ref_scores, 2)[1][0]][
                                      0]) * (ref_boxes[
                                                 torch.topk(ref_scores, 2)[1][0]][3] -
                                             ref_boxes[torch.topk(ref_scores, 2)[1][0]][
                                                 1]))
                    print((
                                  ref_boxes[torch.topk(ref_scores, 2)[1][1]][2] -
                                  ref_boxes[torch.topk(ref_scores, 2)[1][1]][
                                      0]) * (ref_boxes[
                                                 torch.topk(ref_scores, 2)[1][1]][3] -
                                             ref_boxes[torch.topk(ref_scores, 2)[1][1]][
                                                 1]))
                    continue
                draw_PIL_image_2(image, ref_boxes, ref_labels, 'ori', no=torch.topk(ref_scores, 2)[1].data,
                                 color='green')
                ...
                if 'flip' in augs:
                    ...
                    draw_PIL_image_2(flip_image, flip_boxes, ref_labels, '1_', no=torch.topk(ref_scores, 2)[1].data,
                                     color='green')
                if 'multi_ga' in augs:
                    for i in range(1, 7):
                ...
                if 'cut_out' in augs:
                    ...
                    aug_boxes.append(ref_boxes)
                    draw_PIL_image_2(cutout_image, ref_boxes, ref_labels, '2_', no=torch.topk(ref_scores, 2)[1].data,
                                     color='green')
                ...
                if 'smaller_resize' in augs:
                    ...
                    aug_images.append(resize_image.cuda())
                    aug_boxes.append(resize_boxes)
                    draw_PIL_image_2(resize_image, resize_boxes, ref_labels, '3_', no=torch.topk(ref_scores, 2)[1].data,
                                     color='green')
                if 'rotation' in augs:
                    rot_image, rot_boxes = rotate(image, ref_boxes, 5)
                    aug_images.append(rot_image.cuda())
                    aug_boxes.append(rot_boxes)
                    draw_PIL_image_2(rot_image, rot_boxes, ref_labels, '4_', no=torch.topk(ref_scores, 2)[1].data,
                                     color='green')
                ...
                for output, aug_box, aug_image in zip(outputs, aug_boxes, aug_images):
                    ...
                    for ab, ref_score_cls, ref_pm, ref_score in zip(aug_box, ref_scores_cls, prob_max, ref_scores):
                        ...
                        if js < 0:
                            js = 0
                        # print(torch.max(iou),
                        #       0.5 * (1 - js) * (ref_pm + pm[torch.argmax(iou)]))
                        # if 0.7 < torch.max(iou) and 0.4 > 0.5 * (1 - js) * (
                        #         ref_pm + pm[torch.argmax(iou)]) and (ab[2] - ab[0]) > 80 and scores[
                        #     torch.argmax(iou)] > 0.5 and ref_score > 0.6:
                        # if 0.3 < torch.max(iou) < 0.7 and 0.3 < 0.5 * (1 - js) * (
                        #         ref_pm + pm[torch.argmax(iou)]) < 0.7 and (ab[2] - ab[0]) > 60 and scores[
                        #     torch.argmax(iou)] > 0.5 and ref_score > 0.6 and image.size[0] > image.size[1]:
                        #     # draw_PIL_image(image, boxes, ref_labels, i, no=[ab], color='greenyellow')
                        #     no = [j, torch.argmax(iou).item()]
                        #     draw_PIL_image_1(aug_image.cpu(), aug_box, boxes, ref_labels, labels, ref_scores, pm,
                        #                      'bad_3.eps', no=no)
                        #     print(1 / 0)
                        j += 1
                        ...
                        if len(no) < 2:
                            ...
                            draw_PIL_image_2(aug_image.cpu(), boxes, ref_labels, i, no=no, color='red')
                            i += 1
                            break
                    consistency_aug.append(consistency_img)
                    mean_aug.append(np.mean(mean_img))
                print(torch.topk(ref_scores, 2)[1])
                print(1 / 0)
